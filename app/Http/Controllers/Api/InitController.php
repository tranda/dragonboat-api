...
class InitController extends Controller {
    public function index(Request $request) {
        $athletes = Athlete::all()->map(fn($a) => ['id' => $a->id, 'name' => $a->name, 'weight' => $a->weight ?? 0, 'gender' => $a->gender, 'yearOfBirth' => $a->year_of_birth, 'isBCP' => $a->is_bcp, 'preferredSide' => $a->preferred_side, 'notes' => $a->notes, 'isRemoved' => $a->is_removed, 'raceAssignments' => []]);
