...
class UserController extends Controller {
    public function index() {
        return response()->json(User::with('role', 'team')->get()->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'email' => $u->email, 'role' => $u->role->name, 'athlete_id' => $u->athlete_id, 'is_active' => $u->is_active, 'team' => $u->team ? $u->team->name : null, 'team_id' => $u->team_id]));
    }
    public function store(Request $request) {
        ...
        $user = User::create(['name' => $request->name, 'email' => $request->email, 'password' => Hash::make($request->password), 'role_id' => $role->id, 'athlete_id' => $request->athlete_id, 'team_id' => $request->team_id]);
